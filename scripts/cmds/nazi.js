...
module.exports = {
    ...
    onStart: async function({ api, args, message, event }) {
        try {
            ...
            const imageUrl1 = await generateImage(prompt, options);
            console.log("First image URL:", imageUrl1);
            ...
            const imageUrl2 = await generateImage(prompt, options);
            console.log("Second image URL:", imageUrl2);

            // Check if the URL is valid
            if (!imageUrl2) {
                throw new Error("Failed to generate the second image.");
            }

            // Get the image streams
            const imageStream1 = await getStreamFromURL(imageUrl1);
            const imageStream2 = await getStreamFromURL(imageUrl2);

            console.log("Image streams acquired");
